    ...
    return _json(
        {
            ...
            "summary":summary,
            "details":plan,
            "recommended_action": ("Use priority handling" if plan["priority_queue"] else "Use standard queue")
        }
    )
    
@tool
def lookup_open_ticket_load(customer_email: str) -> str:
    """Return the customer's current open-ticket count and workload band."""
    ...
    if not customer:
        return _json({
            ...
            "recommended_action":"Ask agent to verify the customer email before promising SLA"
        })
    ...
    return _json(
        {
            "tool":"lookup_open_ticket_load",
            "customer_email":customer_email,
            "summary":f"{customer_email} has {open_count} open tickets",
            "details":{"customer_found":True,"open_tickets":open_count,"load_band":_load_band(open_count)},
            "recommended_action":("Acknowledge multiple ongoing issue." if open_count>1 else "Handle as isolated incident.")
        }
    )
