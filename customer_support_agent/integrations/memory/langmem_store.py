...
class CustomerMemoryStore:
    def __init__(self,settings:Settings,llm:Any):
        ...
        logger.info("Memory backend initialized: provider=langmem mode=hot_path store=inmemory fallback=fallback_store")
    ...
    def _build_store(self)-> InMemoryStore:
        ...
        try:
            ...
            logger.exception("memory.index.init_failed provider=google_genai model=%s; semantic index disabled",model_name)
            return InMemoryStore()


    def search(self,query:str,user_id:str,limit:int = 5)-> list[dict[str,Any]]:
        logger.info("memory.search.start user=%s limit=%s query=%r",self._namespace_label(user_id),max(1, limit),(query or "").strip()[:120],)
        raw = self._search_items(query=query, user_id=user_id, limit=limit)
        normalized = self._normalize_results(raw, limit=limit)
        logger.info("memory.search.done user=%s returned=%s",self._namespace_label(user_id),len(normalized),)
        return normalized


    def list_memories(self,user_id:str,limit:int = 20)-> list[dict[str,Any]]:
        logger.info("memory.list.start user=%s limit=%s",self._namespace_label(user_id),max(1, limit),)
        raw = self._search_items(query=None, user_id=user_id, limit=limit)
        normalized = self._normalize_results(raw, limit=limit)
        logger.info("memory.list.done user=%s returned=%s",self._namespace_label(user_id),
            len(normalized),
        )
        return normalized
    ...
    def add_interaction(self,user_id:str,user_input:str,assistant_response:str,metadata:dict[str,Any] | None = None) -> None:
        ...
        logger.info("memory.add_interaction.start user=%s chars=%s",self._namespace_label(user_id),len(memory_text),)
        self._create_memory(user_id=user_id, text=memory_text, metadata=metadata)
        logger.info("memory.add_interaction.done user=%s",self._namespace_label(user_id),)
    ...
    def add_resolution(self,user_id:str,ticket_subject:str,ticket_description:str,accepted_draft:str,entity_links:list[str] | None = None)-> None:
        ...
        logger.info("memory.add_resolution.start user=%s subject=%r entities=%s",self._namespace_label(user_id),(ticket_subject or "").strip()[:120],len(entity_links or []))
        self._create_memory(user_id=user_id,text=memory_text,metadata={"type": "resolution"})
        logger.info("memory.add_resolution.done user=%s",self._namespace_label(user_id),)


    def _create_memory(self,user_id:str,text:str,metadata:dict[str,Any] | None = None)-> None:
        clean_text = text.strip()
        if not clean_text:
            logger.warning("memory.create.skipped_empty user=%s",self._namespace_label(user_id))
            return

        config = self._tool_config(user_id=user_id)
        key:str | None = None
        try:
            logger.info("memory.create.attempt system=langmem_tool user=%s",self._namespace_label(user_id))
            ...
            if key:
                logger.info("memory.create.success system=langmem_tool key=%s user=%s",key,self._namespace_label(user_id))
            else:
                logger.warning("memory.create.no_key system=langmem_tool raw_result=%s user=%s",result,self._namespace_label(user_id))
        except Exception:
            logger.exception("memory.create.error system=langmem_tool user=%s; using fallback_store",self._namespace_label(user_id))
            key = None
        ...
        if not key:
            ...
            logger.info("memory.create.success system=fallback_store key=%s user=%s",key,self._namespace_label(user_id))
            return
        if not metadata:
            logger.debug("memory.create.metadata.skip_update system=langmem_tool key=%s user=%s",key,self._namespace_label(user_id))
            return
        item = self._store.get(self._namespace_for_user(user_id), key)
        if not item:
            logger.warning("memory.create.metadata.item_missing user=%s key=%s",self._namespace_label(user_id),key)
            return
    ...
    def _search_items(self,query:str|None,user_id:str,limit:int)->list[Any]:
        ...
        if query and query.strip():
            ...
            if results:
                logger.info("memory.search.path system=langmem_store.semantic user=%s returned=%s",self._namespace_label(user_id),len(results))
                return results
            logger.info("memory.search.path system=langmem_store.semantic user=%s returned=0; fallback=list_recent",self._namespace_label(user_id))

        # Fallback: return latest memories even if query misses.
        fallback = self._store.search(namespace, query=None, limit=safe_limit)
        logger.info("memory.search.path system=fallback_store.list_recent user=%s returned=%s",self._namespace_label(user_id),len(fallback))
        return fallback
