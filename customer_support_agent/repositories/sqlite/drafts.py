...
class DraftsRepository:
    ...
    def update(self,draft_id:int,content:str | None = None,status:str | None = None) -> dict[str,Any]:
    ...
    def get_ticket_and_customer_by_draft(self,draft_id:int)-> dict[str,Any]:
        with connect() as conn:
            row = conn.execute("""
                SELECT 
                d.id AS draft_id,
